...
export async function runNotificationsLayer ({ runCase }: ModuleTools): Promise<void> {
	await runCase('Instant players receive wrong-turn mailings in real time', async () => {
		...
		assertSent(getLog(), CAROL.id, 'Количество не 1');
	});
	...
	await runCase('Composed players are excluded from real-time wrong-turn mailings', async () => {
		...
		assertNotSent(getLog(), CAROL.id, 'Количество не 1');
	});
	...
	await runCase('Composed players receive a last-round summary before their turn prompt', async () => {
		...
		assert(aliceMessages.length === 2, 'Composed player should receive a summary and then a turn prompt');
		assert(aliceMessages[0]!.includes('Вот что было за последний круг'), 'First message should be the composed summary');
		assert(aliceMessages[1]!.includes('Твой ход!'), 'Second message should be the regular turn prompt');
	});
	...
	await runCase('Last-round summary stops at the active player’s previous turn', async () => {
		...
		assert(logs.includes('<b>Каролина -> Борис</b> | 3 | Не 1'), 'Logs after Alice’s previous turn should be included');
